...
class VoterCommands(commands.Cog):
        ...
        if ctx.invoked_subcommand is None:
            ...
            list_of_values = [
                ...
                {"name": ":ballot_box: Cast vote to project",
                 "value": f"```{self.guild_string}vote FOR/AGAINST <ballot ID> <amount>```"},
        ...
        if vote in ["FOR", "AGAINST"]:
            ...
            if self.bot.backoffice.voting_manager.check_ballot_id(ballot_id=ballot_id):
                ...
                user_balance = self.bot.backoffice.wallet_manager.get_ticker_balance(
                    asset_code=ballot_data["assetCode"],
                    user_id=ctx.message.author.id)

                # Converting amount to atomic
                amount_atomic = amount * (int(amount * (10 ** 7)))
                if user_balance > amount_atomic:
                    ...
                    if self.bot.backoffice.wallet_manager.update_coin_balance(coin=ballot_data["assetCode"].lower(),
                        ...
                        if vote == "FOR":
                            ...
                            # Update list voter for
                            new_voter_for = ballot_data["voterFor"].append({"voterId": ctx.author.id,
                                                                            "voterName": f'{ctx.author}',
                                                                            "votePwr": amount_atomic})
                            new_ballot_data.update({"toUpdate": {"voterFor": new_voter_for}})

                        else:
                            new_ballot_data.update({"toIncrement": {"votesAgainst": amount_atomic}})

                            # Update list voter for
                            new_voter_against = ballot_data["voterAgainst"].append({"voterId": ctx.author.id,
                                                                                    "voterName": f'{ctx.author}',
                                                                                    "votePwr": amount_atomic})
                            new_ballot_data.update({"toUpdate": {"voterAgainst": new_voter_against}})
    ...
    async def ballot(self, ctx):
        if ctx.invoked_subcommand is None:
            self.guild_string = self.bot.get_prefix_help(ctx.guild.id)
            description = f"All commands concerning ballot boxes on {ctx.guild}"
            title = ':joystick: __Welcome to Ballot Bpx __ :joystick: '
            list_of_values = [
                {"name": ":information_source: About ballot system",
                 "value": f"```{self.guild_string}ballot about```"},
                {"name": f":ballot_box: All live ballots on {ctx.guild}",
                 "value": f"```{self.guild_string}ballot live```"},
